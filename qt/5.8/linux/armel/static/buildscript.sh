...
sed -i -E -e 's|load\(qt_config\)||g' qtbase/mkspecs/linux-arm-gnueabi-g++/qmake.conf
echo "QMAKE_CFLAGS += -marm -mfpu=vfp -march=armv5 -mabi=aapcs-linux" >> qtbase/mkspecs/linux-arm-gnueabi-g++/qmake.conf
echo "QMAKE_CXXFLAGS = \$\$QMAKE_CFLAGS" >> qtbase/mkspecs/linux-arm-gnueabi-g++/qmake.conf
echo "load(qt_config)" >> qtbase/mkspecs/linux-arm-gnueabi-g++/qmake.conf
